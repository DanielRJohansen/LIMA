...
__device__ void calcSinglebondForces(const Float3& pos_a, const Float3& pos_b, SingleBond& bondtype, Float3* results, float& potE) {
...
__device__ static Float3 calcLJForceOptim(const Float3& pos0, const Float3& pos1, float* data_ptr, float& potE, const float sigma, const float epsilon, 
	...
	if constexpr (HARD_CUTOFF) {
		...
		if (dist_sq > 1.8f * 1.8f * NANO_TO_LIMA*NANO_TO_LIMA) {
			return Float3{ 0,0,0 };
		}
	}
