...
namespace NListUtils {
	...
	void updateNeighborLists(Simulation* simulation, NListDataCollection* nlist, volatile bool* finished, int* timing, bool* mutex_lock, const uint32_t step_at_update) {
		auto t0 = std::chrono::high_resolution_clock::now();
		EngineUtils::genericErrorCheck("1");
		// Make key positions addressable in arrays: compound_key_positions and solvent_positions
		nlist->preparePositionData(*simulation, step_at_update);

		// First do culling of neighbors that has left CUTOFF
		NListUtils::cullDistantNeighbors(simulation, nlist);
		...
		for (uint16_t id_self = 0; id_self < simulation->n_compounds; id_self++) {
			...
			const Float3& pos_self = nlist->compound_key_positions[id_self];
			...
			for (uint16_t id_other = id_self + 1; id_other < simulation->n_compounds; id_other++) {	// For finding new nearby compounds, it is faster and simpler to just check all compounds, since there are so few
				NeighborList* nlist_candidate = &nlist->compound_neighborlists[id_other];
				const Float3& pos_other = nlist->compound_key_positions[id_other];
		...
		EngineUtils::genericErrorCheck("3");
		updateCompoundGrid(simulation, nlist);
		EngineUtils::genericErrorCheck("4");
		Int3 after(nlist->compound_neighborlists[0].n_compound_neighbors, nlist->compound_neighborlists[0].n_solvent_neighbors, 0);
...
void NListUtils::updateCompoundGrid(Simulation* simulation, NListDataCollection* nlist) {
	...
	EngineUtils::genericErrorCheck("5");
	auto a = nlist->compoundgrid->getOrigosPtr();
	auto b = simulation->box->compound_grid->getOrigosPtr();
	auto s = sizeof(CompoundGrid);

	auto* cg = new CompoundGrid();
...
//		sizeof(Coord)*2,
	...
	EngineUtils::genericErrorCheck("6");
	NListUtils::distributeCompoundsInGrid(simulation, nlist->compoundgrid);
	NListUtils::assignNearbyCompoundsToGridnodes(simulation, nlist);
	EngineUtils::genericErrorCheck("7");
...
void NListUtils::distributeCompoundsInGrid(Simulation* simulation, CompoundGrid* compoundgrid_host) {
...
bool NListUtils::isNearby(const Simulation& simulation, const Coord& node_origo, const int querycompound_id, CompoundGrid* compoundgrid_host) {
...
void NListUtils::assignNearbyCompoundsToGridnodes(Simulation* simulation, NListDataCollection* nlist) {
	for (int z = 0; z < CompoundGrid::blocks_per_dim; z++) {
		for (int y = 0; y < CompoundGrid::blocks_per_dim; y++) {
			for (int x = 0; x < CompoundGrid::blocks_per_dim; x++) {
				...
				auto node_self = nlist->compoundgrid->getBlockPtr(node_origo);
				...
				for (int x = -query_range; x <= query_range; x++) {
					for (int y = -query_range; y <= query_range; y++) {
						for (int z = -query_range; z <= query_range; z++) {
							...
							const auto node_query = nlist->compoundgrid->getBlockPtr(query_origo);

							for (int i = 0; i < node_query->n_associated_compounds; i++) {
								const int querycompound_id = node_query->associated_ids[i];

								if (isNearby(*simulation, node_origo, querycompound_id, nlist->compoundgrid)) {

									node_self->addNearbyCompound(querycompound_id);	// Add compound so solvents can see it
									nlist->compound_neighborlists[querycompound_id].addGridnode(nodeself_id);	// Add grid so compound can see solvents
								}
							}
						}
					}
...
void NListUtils::transferCompoundgridToDevice(Simulation* simulation, CompoundGrid* compoundgrid_host) {
